...
(let ((elves (make-hash-table :test 'eq))
      (intervals (make-hash-table :test 'eq))
      (times (make-hash-table :test 'eq)))
  (loop
     :with id :and asleep-minutes
     :for time-slot :in *schedule*
     :do
       (let* ((info (nthcdr 5 time-slot))
              (minute (car (nthcdr 4 time-slot)))
              (action (car info)))
         (cond
           ((eq 'Guard action)
            (when id
              (push asleep-minutes (gethash id elves)))
            (setf id (cadr info))
            (setf asleep-minutes nil))
           ((eq 'falls action)
            (push minute asleep-minutes))
           ((eq 'wakes action)
            (push (cons (car asleep-minutes) minute) (gethash id intervals))
            (push (- minute (pop asleep-minutes)) asleep-minutes)))))
  (loop
     :for asleep-minutes :being :the :hash-values :in elves :using (hash-key id)
     :do
       (let ((flat-times (apply #'concatenate 'list asleep-minutes)))
         (setf (gethash id elves) flat-times)
         (setf (gethash id times) (apply #'+ flat-times))))
  (loop
     :with max-minutes := 0 :and max-id
     :for minutes :being :the :hash-value :in times :using (hash-key id)
     :do
       (when (< max-minutes minutes)
         (setf max-minutes minutes
               max-id id))
     :finally (return (values max-id max-minutes))) ;; => 857 651

  (let ((intervals-of-857 (gethash 857 intervals))
        (minutes (make-array 60 :initial-element 0)))
    (loop
       :for (k . l) :in intervals-of-857
       :do (loop :for m :from k :below l :do (incf (aref minutes m))))
    (loop
       :with minute :and count := 0
       :for i :from 0 :below 60
       :when (> (aref minutes i) count)
       :do (setf minute i count (aref minutes i))
       :finally (return minute))) ;; => 46

  (* 46 857) ;; => 39422

  (loop
     :with gminute :and gcount := 0 :and gid
     :for intervals :being :the :hash-value :in intervals :using (hash-key id)
     :do (let ((minutes (make-array 60 :initial-element 0)))
           (loop
           ...
           (loop
              ...
              :when (> (aref minutes i) count)
              :do
                (setf minute i count (aref minutes i))
              :finally
                (when (> count gcount)
                  (setf gcount count
                        gminute minute
                        gid id))))
     :finally
       (return (values gminute gid)))) ;; 38 1723
