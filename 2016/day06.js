...
let messages = fs.readFileSync(0, {encoding: 'utf-8'})
  .trim()
  .split('\n')

let freqs = Array(messages[0].length).fill('')

function letter_counts(word) {
  let bag = new Map
  for (let i = 0; i < word.length; i++) {
    let count = bag.get(word[i]) ?? 0
    bag.set(word[i], count + 1)
  }
  return bag
}

for (let c = 0; c < messages[0].length; c++) {
  for (let r = 0; r < messages.length; r++) {
    freqs[c] += messages[r][c]
  }
}

console.log(freqs.map((word) => Array.from(letter_counts(word)).sort((a, b) => b[1] - a[1])[0][0]).join(''))
...
// column is a; in the second, d, and so on. Repeating this process
...
console.log(freqs.map((word) => Array.from(letter_counts(word)).sort((a, b) => a[1] - b[1])[0][0]).join(''))
