...
// the instructions to decode the list are barely hidden
...
let rooms = fs.readFileSync(0, {encoding: 'utf-8'})
  .trim()
  .split('\n')
  .map(room => room.split(/[\[\]-]+/).reverse())


let sum = 0
for (let [_, checksum, sector, ...chunks] of rooms) {
  let counts = chunks.join('').split('').sort().reduce((bag, letter) => {
    return {
      ...bag,
      [letter]: bag[letter] ? bag[letter]+1 : 1
    }
  }, {})

  let ordered = Object.entries(counts).sort((a, b) => {
    const order = b[1] - a[1]
    if (order == 0)
  ...
  if (ordered.slice(0, 5).map(x => x[0]).join('') == checksum) {
    sum += Number(sector)
  }
}
console.log(sum)
...
for (let [_, checksum, sector, ...chunks] of rooms) {
  ...
  let name = chunks.map(chunk => chunk.split('').map(c => rot(c, sector)).join('')).join(' ')
  if (name.includes('north'))
    console.log( sector )
}

function rot(c, n) {
  return String.fromCharCode(97 + (((c.charCodeAt(0) - 97) + n) % 26))
}
